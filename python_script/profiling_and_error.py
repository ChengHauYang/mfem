...
SOLVERS = {
    "ceed-amg": ("CEED AMG", ("-pa", "-no-fa", "-d", "ceed-cpu", "-a")),
    "pa-jacobi": (
        "PA Jacobi",
        ("-pa", "-no-fa", "-d", "ceed-cpu", "-no-a"),
    ),
    "hypre-amg": (
        "Hypre BoomerAMG",
        ("-no-pa", "-no-fa", "-d", "cpu", "-no-a"),
    ),
}
...
def run_once(
    ...
    if process.returncode:
        raise RuntimeError(
            f"{solver}, order={order}, n={size}, run={run} failed:\n{output}"
        )
    ...
    if not dofs or not error or not iterations or not elapsed:
        raise RuntimeError(
            f"could not parse {solver}, order={order}, n={size}, run={run}:\n{output}"
        )
    return (
        int(dofs.group(1)),
        float(error.group(1)),
...
def profile_case(
    ...
    case_dir = work_dir / solver / f"order{order}" / f"n{size:04d}"
    case_dir.mkdir(parents=True, exist_ok=True)
    print(f"Profiling {SOLVERS[solver][0]}, p={order}, n={size} ...", flush=True)

    # Warm up dynamic libraries, kernels, and allocator state before timing samples.
    run_once(executable, case_dir, mpi_ranks, solver, mms, order, size, 0)
    samples = []
    iteration_samples = []
    dofs = 0
    l2_error = 0.0
    for run in range(1, repeats + 1):
        dofs, l2_error, iterations, elapsed = run_once(
            executable, case_dir, mpi_ranks, solver, mms, order, size, run
        )
...
def write_plot(results: list[ProfileResult], path: Path) -> None:
    ...
    for order, color in zip(DEFAULT_ORDERS, colors):
        for solver, (linestyle, marker) in styles.items():
            series = sorted(
                (
                    result
                    for result in results
                    if result.order == order and result.solver == solver
                ),
    ...
    axis.set_xlabel(r"$\textrm{Preconditioner setup + CG solve time (s)}$", fontsize=14)
    axis.set_ylabel(r"$L^2\textrm{ error}$", fontsize=14)
    axis.set_title(r"$\textrm{Time-to-Accuracy Comparison}$", fontsize=17)
...
def main() -> None:
        ...
        for solver in args.solvers:
            for order in args.orders:
                ...
                for size in sizes:
                    ...
                    raw_rows.extend(
                        RawSample(solver, args.mms, order, size, run, iterations, elapsed)
    ...
    for result in results:
        print(
            f"{result.solver:11s} p={result.order} n={result.n:3d} "
