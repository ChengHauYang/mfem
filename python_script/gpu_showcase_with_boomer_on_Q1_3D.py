#!/usr/bin/env python3
"""Run the 3D GPU showcase with an additional CEED AMG Q1 BoomerAMG case."""
...
DIM = 3
# p6 n=8 is 117,649 DOFs on the unit cube, which the assembled p6 Hypre
# BoomerAMG case can still hold; p1 n=64 is 274,625 DOFs, keeping the same
# "p1 slightly larger than p6" balance the 2D showcase uses.
DEFAULT_P6_SIZE = 8
DEFAULT_P1_SIZE = 64
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--p1-size",
        type=int,
        default=DEFAULT_P1_SIZE,
        help=f"elements per side for the p1 PA Jacobi case "
             f"(default: {DEFAULT_P1_SIZE})",
    )
    parser.add_argument(
        "--p6-size",
        type=int,
        default=DEFAULT_P6_SIZE,
        help=f"elements per side for all p6 solver cases "
             f"(default: {DEFAULT_P6_SIZE})",
    )
...
def main() -> None:
    ...
    results = []
    with tempfile.TemporaryDirectory(prefix="mfem-gpu-showcase-q1-3d-") as temporary_dir:
    ...
    csv_path = args.output / tagged("gpu_showcase_with_boomer_on_Q1_3D", ".csv")
    comparison_path = args.output / tagged(
        "gpu_showcase_with_boomer_on_Q1_3D", ".png"
    )
    phase_path = args.output / tagged(
        "gpu_showcase_with_boomer_on_Q1_3D_phases", ".png"
    )
